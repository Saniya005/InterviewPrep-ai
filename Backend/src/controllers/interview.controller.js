...
async function regenerateRoadmapController(req, res) {
    try {
        ...
        const newPlan = await generateRoadmap({ resume, jobDescription, selfDescription, days })

        interviewReport.preparationPlan = newPlan
        await interviewReport.save()

        res.status(200).json({
            message: "Roadmap regenerated successfully.",
            preparationPlan: newPlan
        })
    } catch (err) {
...
module.exports = { generateInterViewReportController, getInterviewReportByIdController, getAllInterviewReportsController, generateResumePdfController, regenerateRoadmapController }